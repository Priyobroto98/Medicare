...
#GEN AI diet and work out recommendation model

os.environ["GOOGLE_API_KEY"] = 'AIzaSyD3XwLkXOJXrV_4VYOTwgcpYPA5D5_UOmM'
...
def recommend():
    if request.method == "POST":
            ...
            foodtype = request.form['veg_or_nonveg']
            disease = request.form['disease']
            diability_if_any = request.form['diability_if_any']
            ...
            # Regular expressions to extract recommendations
            ...
            breakfast_matches = breakfast_pattern.search(input_text)
            dinner_matches = dinner_pattern.search(input_text)
            workout_matches = workout_pattern.search(input_text)

            # Clean up the extracted lists
            def clean_recommendations(recommendations):
                return [line.strip() for line in recommendations.split("\n") if line.strip()]

            breakfast_recommendations = clean_recommendations(breakfast_matches.group(1))
            dinner_recommendations = clean_recommendations(dinner_matches.group(1))
            workout_recommendations = clean_recommendations(workout_matches.group(1))

            return render_template('result.html',breakfast_recommendations=breakfast_recommendations,dinner_recommendations=dinner_recommendations,workout_recommendations=workout_recommendations)
    return render_template('diet.html')
...
def visual():
    ...
    return render_template("visual_search.html",response=response,uploaded_file=uploaded_file)
...
def posture():
    if request.method == "POST":
        ...
        response = response.split('\n')
    return render_template("posture_detection.html",response=response)
